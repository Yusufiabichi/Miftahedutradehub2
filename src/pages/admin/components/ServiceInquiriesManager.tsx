...
export default function ServiceInquiriesManager() {
  ...
  const fetchInquiries = useCallback(async () => {
    try {
      ...
      setFetchError('Unable to load service inquiries. Please try again.');
      setInquiries([]);
    } finally {
      setLoading(false);
    }
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/enquiries/service/${deletingInquiry.id}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
  ...
  const serviceNames = useMemo(
    () => ['all', ...Array.from(new Set(inquiries.map((i) => i.service_name).filter(Boolean)))],
    [inquiries]
  );
  ...
  const filteredInquiries = useMemo(() => {
    ...
    return inquiries.filter((i) => {
      const matchesService = serviceFilter === 'all' || i.service_name === serviceFilter;
      const matchesSearch =
        !q ||
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
          <h2 className="text-2xl font-bold text-blue-900">Service Inquiries</h2>
        ...
        <div className="mb-6 flex flex-col gap-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search by name, service, or message..."
            ...
            <div className="flex flex-wrap gap-2">
              {serviceNames.map((service) => {
                ...
                const count =
                  service === 'all'
                    ? inquiries.length
                    : inquiries.filter((i) => i.service_name === service).length;
                return (
                  <button
                    key={service}
      ...
      {loading && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="grid grid-cols-5 gap-4 px-6 py-4 border-b border-gray-100 last:border-0"
            >
      ...
      {!loading && !fetchError && inquiries.length === 0 && (
        <div className="text-center py-20 bg-white border border-dashed border-gray-200 rounded-xl">
          <div className="w-20 h-20 bg-teal-50 rounded-full flex items-center justify-center mx-auto mb-5">
            <i className="ri-question-answer-line text-4xl text-teal-600" aria-hidden="true"></i>
          </div>
          <h3 className="text-xl font-bold text-blue-900 mb-2">No service inquiries yet</h3>
          <p className="text-gray-600 max-w-sm mx-auto">
            When customers submit a service inquiry, it will appear here.
          </p>
        </div>
      )}
      ...
      {!loading && !fetchError && inquiries.length > 0 && filteredInquiries.length === 0 && (
        <div className="text-center py-16 bg-white border border-gray-100 rounded-xl">
          ...
          <p className="text-gray-600 text-sm mb-5">Try a different search or filter.</p>
          <button
            onClick={() => {
              setSearch('');
                ...
                {filteredInquiries.map((inquiry) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm font-semibold text-blue-900">{inquiry.name}</div>
        ...
        >
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[92vh] overflow-y-auto shadow-2xl">
            ...
            <div className="px-6 sm:px-8 py-6 space-y-6">
              ...
              {/* Project details */}
              {(selectedInquiry.budget_range || selectedInquiry.expected_timeline) && (
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 px-6 sm:px-8 py-4 flex flex-wrap justify-between items-center gap-3">
              <a
                href={`mailto:${selectedInquiry.email}?subject=${encodeURIComponent(
                  `Re: ${selectedInquiry.service_name}`
                )}&body=${encodeURIComponent(
                  `Hello ${selectedInquiry.name},\n\nThank you for your interest in ${selectedInquiry.service_name}.\n\nBest regards,\nMiftah Edu-Trade Hub`
                )}`}
      ...
      {deletingInquiry && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="service-inquiry-delete-title"
          onClick={(e) => {
        ...
        >
          <div className="bg-white rounded-2xl max-w-md w-full p-6 sm:p-8 shadow-2xl">
            ...
            <h3
              id="service-inquiry-delete-title"
            ...
            <p className="text-gray-900 font-semibold text-center mb-5 truncate">
              {deletingInquiry.name} · {deletingInquiry.service_name}
            </p>
